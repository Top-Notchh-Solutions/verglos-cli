...
test("local boundary journey composes diff, header, and viewer without hosted dependencies", () => {
  ...
  const base = createReleaseSnapshot({ primarySubject: subject, subjects: [subject], observations: [], lineage: { edges: [], gaps: [] }, policyInputs: { mode: "free" }, coverage: coverage(0) });
  const head = createReleaseSnapshot({ primarySubject: subject, subjects: [subject], observations: [{ fingerprint: `sha256:${"c".repeat(64)}`, producerIds: ["native"], payloads: [], disagreement: false }], lineage: { edges: [], gaps: [] }, policyInputs: { mode: "free" }, coverage: coverage(1) });
