...
import { parseReleaseRecordManifestJson, readAndVerifyRecord, releaseRecordManifestDigest, verifyReleaseRecordSignature } from "@verglos/shared";
...
export async function executeRecordVerify(root: string, manifestPath: string, json = false, quiet = false, signaturePath?: string, publicKeyPath?: string): Promise<number> {
  try {
    ...
    const result = { verified: true, manifestDigest: releaseRecordManifestDigest(manifest), members: members.size, paths: [...members.keys()].sort(), ...(signature ? { signature } : {}) };
