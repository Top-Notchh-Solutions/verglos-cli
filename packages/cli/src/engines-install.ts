...
export async function executeEngineInstall(engineId: string, version: string, artifactPath: string, digest: string, options: { action?: "install" | "update" | "rollback"; approve?: boolean; json?: boolean; quiet?: boolean } = {}): Promise<number> {
  try {
