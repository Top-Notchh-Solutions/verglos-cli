...
export async function executePolicyCheck(path: string, json = false, quiet = false, options: PolicyCheckOptions = {}): Promise<number> {
  try {
    ...
    if (json) console.log(JSON.stringify({ status: "error", code: "POLICY_CHECK_INPUT", message: "policy check failed" })); else if (!quiet) console.error(`[POLICY_CHECK_INPUT] ${message}`);
    return 2;
  }
}
