...
export const RELEASE_RECORD_PACKAGE_SCHEMA = { id: "urn:verglos:schema:release-record-package", version: "1.1.0" } as const;
const LEGACY_RELEASE_RECORD_PACKAGE_VERSION = "1.0.0" as const;
export const RELEASE_RECORD_PACKAGE_FILES = {
  ...
  sigstoreBundle: ".vgl-sigstore.json",
  sigstoreBinding: ".vgl-sigstore-binding.json",
} as const;

const LegacyDescriptorSchema = z.object({
  schemaId: z.literal(RELEASE_RECORD_PACKAGE_SCHEMA.id),
  schemaVersion: z.literal(LEGACY_RELEASE_RECORD_PACKAGE_VERSION),
  transport: z.literal("directory-v1"),
  manifestDigest: z.string().regex(/^sha256:[a-f0-9]{64}$/u),
  signatureIncluded: z.boolean(),
}).strict();
const SigstoreDescriptorSchema = z.object({
  schemaId: z.literal(RELEASE_RECORD_PACKAGE_SCHEMA.id),
  schemaVersion: z.literal(RELEASE_RECORD_PACKAGE_SCHEMA.version),
  transport: z.literal("directory-v1"),
  manifestDigest: z.string().regex(/^sha256:[a-f0-9]{64}$/u),
  signatureIncluded: z.boolean(),
  sigstoreIncluded: z.literal(true),
}).strict();
const DescriptorSchema = z.discriminatedUnion("schemaVersion", [LegacyDescriptorSchema, SigstoreDescriptorSchema]);

export type ReleaseRecordPackageDescriptor = z.infer<typeof DescriptorSchema>;

export function createReleaseRecordPackageDescriptor(manifest: ReleaseRecordManifestDocument, signatureIncluded: boolean, sigstoreIncluded = false): ReleaseRecordPackageDescriptor {
  const parsed = parseReleaseRecordManifest(manifest);
  if (!sigstoreIncluded) {
    return LegacyDescriptorSchema.parse({
      schemaId: RELEASE_RECORD_PACKAGE_SCHEMA.id,
      schemaVersion: LEGACY_RELEASE_RECORD_PACKAGE_VERSION,
...
export function renderReleaseRecordViewer(input: {
  ...
  readonly sigstoreIncluded?: boolean;
}): string {
  ...
  const rows = [
    ...fields.map(([label, value]) => `<dt>${escapeHtml(label)}</dt><dd>${escapeHtml(value)}</dd>`),
    ...(input.sigstoreIncluded ? ['<dt>Sigstore bundle</dt><dd>Included; trust and exact signer identity are not verified by this static viewer.</dd>'] : []),
  ].join("\n");
  const signatureGuidance = input.sigstoreIncluded
    ? "<p>This is a limited, static summary. Verify the original package signatures against separately trusted material before relying on signer identity.</p>"
