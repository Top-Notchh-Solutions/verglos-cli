...
export const PolicyDocumentSchema = z.object({
  ...
  extensions: z.record(z.string().regex(/^urn:verglos:extension:/), z.unknown()).optional(),
}).strict().superRefine((value, ctx) => {
...
export function parsePolicyDocument(value: unknown): PolicyDocument {
  ...
  if (!parsed.success) throw new Error(`Invalid policy document: ${parsed.error.issues.map((issue) => issue.message).join("; ")}`);
  return parsed.data;
}
