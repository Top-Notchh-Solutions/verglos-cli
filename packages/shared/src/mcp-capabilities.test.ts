...
test("MCP capability truth labels plan and maturity", () => {
  ...
  assert.deepEqual(capabilities.find((item) => item.tool === "verglos_scan"), { tool: "verglos_scan", action: "inspect", plan: "free", maturity: "shipped", approvalRequired: false, sideEffect: "none", inputFields: ["projectRoot", "limit", "noProvenance"], outputFields: ["projectRoot", "scannedAt", "durationMs", "score", "provenance", "findingCount", "findings", "truncated", "headline"] });
  ...
  assert.deepEqual(capabilities.find((item) => item.tool === "verglos_scan")?.inputFields, ["projectRoot", "limit", "noProvenance"]);
