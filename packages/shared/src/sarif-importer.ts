...
export function importSarif(bytes: Uint8Array): ImportedSarif {
  ...
  for (const run of runs as Record<string, unknown>[]) { const results = run.results; if (results !== undefined && !Array.isArray(results)) throw new SarifImportError("MALFORMED_RUN", "SARIF results must be an array."); if (Array.isArray(results)) for (const result of results as Record<string, unknown>[]) { const locations = result.locations; if (locations !== undefined && !Array.isArray(locations)) throw new SarifImportError("MALFORMED_RUN", "SARIF locations must be an array."); for (const location of (locations as Record<string, unknown>[] | undefined) ?? []) { const uri = (location.physicalLocation as Record<string, unknown> | undefined)?.artifactLocation as Record<string, unknown> | undefined; if (uri?.uri !== undefined && typeof uri.uri === "string") { try { const parsed = new URL(uri.uri); if (parsed.protocol !== "https:" || parsed.username || parsed.password) throw new Error(); } catch { if (uri.uri.startsWith("/") || uri.uri.split("/").includes("..")) throw new SarifImportError("MALFORMED_URI", "SARIF artifact URI is unsafe."); } } } } }
