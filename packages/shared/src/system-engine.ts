...
export async function inspectSystemEngine(path: string): Promise<SystemEngineInspection> {
  ...
  let bytes: Buffer; try { const stat = await lstat(path); if (!stat.isFile() || stat.size > MAX_SYSTEM_ENGINE_BYTES) throw new Error(); bytes = await readFile(path); } catch { throw new SystemEngineError("NOT_EXECUTABLE", "Explicit system engine path is not a readable file."); }
