...
test("Hunt execution requires trusted exact recipe and execute approval", () => {
  ...
  assert.equal(canExecuteHunt(recipe, { ruleId: "d1-1", subjectId, at: "2026-01-02T00:00:00Z", approval }, { ...trust, signedFeeds: [] }), false);
  ...
  const allowlistedRecipe = parseHuntRecipe({ ...recipe, limits: { ...recipe.limits, maxNetworkRequests: 1 }, network: { mode: "allowlist", destinations: ["https://example.com"], reason: "fixture" }, isolation: "container" });
  ...
  const allowlistedTrust = createTestHuntTrustStore(allowlistedRecipe);
  assert.equal(canExecuteHunt(allowlistedRecipe, { ruleId: "d1-1", subjectId, at: "2026-01-02T00:00:00Z", approval: allowlistedApproval }, allowlistedTrust), true);
  ...
  assert.equal(canExecuteHunt(allowlistedRecipe, { ruleId: "d1-1", subjectId, at: "2026-01-02T00:00:00Z", approval: widenedApproval }, allowlistedTrust), false);
