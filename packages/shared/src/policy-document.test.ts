...
import { parsePolicyDocument, policyDocumentDigest, PolicyDocumentValidationError } from "./policy-document.js";
...
test("policy documents validate strict gates and produce stable digests", () => {
  ...
  assert.throws(() => parsePolicyDocument({ ...policy, policyVersion: "1\u0000" }), (error: unknown) => error instanceof PolicyDocumentValidationError);
  assert.throws(() => parsePolicyDocument({ ...policy, extensions: Object.fromEntries(Array.from({ length: 65 }, (_, i) => [`urn:verglos:extension:${i}`, true])) }), (error: unknown) => error instanceof PolicyDocumentValidationError);
});
