...
export interface ImportedCycloneDx { readonly format: "cyclonedx"; readonly version: string; readonly serialNumber?: string; readonly sourceDigest: ImportedDocument["sourceDigest"]; readonly components: readonly Record<string, unknown>[]; readonly dependencies: readonly Record<string, unknown>[]; readonly document: Record<string, unknown>; }
export class CycloneDxImportError extends Error { override readonly name = "CycloneDxImportError"; constructor(readonly code: "MALFORMED_COMPONENT" | "UNSUPPORTED_VERSION", message: string) { super(message); } }
export function importCycloneDx(bytes: Uint8Array): ImportedCycloneDx {
