...
test("Hunt planner is exact-match and execution-free", () => { const plan = planHunt(recipe, { ruleId: "d1-1", subjectId: recipe.targetSubjectId }); assert.equal(plan.supported, true); assert.equal(plan.executes, false); assert.deepEqual(plan.command, ["node", "check.js"]); assert.equal(planHunt(recipe, { ruleId: "other", subjectId: recipe.targetSubjectId }).supported, false); });
