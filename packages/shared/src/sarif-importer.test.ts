...
test("SARIF importer rejects malformed nested run structures", () => {
  const encode = (value: unknown) => new TextEncoder().encode(JSON.stringify({ version: "2.1.0", runs: [{ results: value }] }));
  assert.throws(() => importSarif(encode([null])), (e: unknown) => e instanceof SarifImportError && e.code === "MALFORMED_RUN");
  assert.throws(() => importSarif(encode([{ locations: [null] }])), (e: unknown) => e instanceof SarifImportError && e.code === "MALFORMED_RUN");
  assert.throws(() => importSarif(encode([{ locations: [{ physicalLocation: { artifactLocation: { uri: 42 } } }] }])), (e: unknown) => e instanceof SarifImportError && e.code === "MALFORMED_URI");
});
