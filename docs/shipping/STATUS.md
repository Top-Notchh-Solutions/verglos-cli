...
`AGENT-005`: MCP read-only validators now cover check-before-write, explain-finding, check-package, and scan argument boundaries. Direct tool calls now reject malformed and unknown fields; package evidence gaps and deterministic scan ordering remain explicit. Scanner network/runtime safety and full tool contract remain open.
