...
- [ ] **ENGINE-010** · Trivy JSON parser/normalizer · deps: ENGINE-009, CONTRACT-004 · Treat output as untrusted bounded input, preserve raw bytes and namespaced extensions, and map to canonical observations. Commit(s): `pending (preparatory)`
- [ ] **ENGINE-011** · Trivy attribution and run metadata · deps: ENGINE-010 · Record binary/database/check/config digests, timestamps, capabilities, license, and source in every run/view/export. Commit(s): `pending (preparatory)`
