...
test("publish workflow verifies the same public artifact boundaries as release checks", () => {
  ...
  assert.match(workflow, /sha256sum \.\/\*\.tgz \.\/THIRD_PARTY_NOTICES \.\/SBOM\.cdx\.json \.\/SBOM\.spdx\.json \.\/dependency-license-inventory\.json \.\/clean-consumer-verification\.txt > SHA256SUMS/u);
